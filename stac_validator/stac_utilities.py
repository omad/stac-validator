...
class StacVersion:
    ...
    def _determine_verison(self):
        ...
        if requests.get(cdn_base_url).status_code == 200:
            self.ITEM_URL = posixpath.join(cdn_base_url, self.filename)
            self.CATALOG_URL = posixpath.join(cdn_base_url, self.filename)
            self.COLLECTION_URL = posixpath.join(cdn_base_url, self.filename)
        else:
            if self.version in old_versions:
                self.CATALOG_URL = posixpath.join(git_base_url, f"static-catalog/{self.input_type}/{self.filename}")
                self.ITEM_URL = posixpath.join(git_base_url, f"json-spec/{self.input_type}/{self.filename}")

            else:

                self.CATALOG_URL = posixpath.join(git_base_url, f"catalog-spec/{self.input_type}/{self.filename}")
                self.COLLECTION_URL = posixpath.join(git_base_url, f"collection-spec/{self.input_type}/{self.filename}")
                self.ITEM_URL = posixpath.join(git_base_url, f"item-spec/{self.input_type}/{self.filename}")
